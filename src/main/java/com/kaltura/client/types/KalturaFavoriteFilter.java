...
public class KalturaFavoriteFilter extends KalturaFilter {
    ...
    public int mediaTypeEqual = Integer.MIN_VALUE;
	/**  Media identifiers from which to filter the favorite assets  */
    public String mediaIdIn;

    public KalturaFavoriteFilter() {
    }

    public KalturaFavoriteFilter(Element node) throws KalturaApiException {
        super(node);
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (nodeName.equals("mediaTypeEqual")) {
                this.mediaTypeEqual = ParseUtils.parseInt(txt);
                continue;
            } else if (nodeName.equals("mediaIdIn")) {
                this.mediaIdIn = ParseUtils.parseString(txt);
                continue;
            } 
        }
    }
